...
public class CustomStudentControllerAdvice {
    ...
    public ResponseEntity<ErrorResponse> handleException(StudentNotExistException e) {
        setErrBody(e);
        return new ResponseEntity<>(new ErrorResponse(status, e.getMessage(), stackTrace, "student01"), status);
    }
    ...
    public ResponseEntity<ErrorResponse> handleException(StudentNotSavedException e) {
        setErrBody(e);
        return new ResponseEntity<>(new ErrorResponse(status, e.getMessage(), stackTrace, "student02"), status);
    }
